...
def clean_json_string(raw: str) -> str:
    """
    Attempt to clean a potentially malformed JSON string by fixing common issues:
    - Strip BOM
...
# ADDED: New helper function to calculate the next sequential ID
def get_next_id(table_name: str) -> str:
    """
    Calculates the next sequential string ID for a given table.
    It finds the max current ID, increments it, and returns the result as a string.
    """
    conn = sqlite3.connect('recruitment.db')
    cursor = conn.cursor()
...
def format_and_save_processed_data(processed_data: str) -> str:
    ...
    Applies clean_json_string() to attempt fixing malformed JSON first.
    """
    try:
        ...
        if document_type == 'CV':
            # MODIFIED: Call helper to get the next sequential ID as a string
            new_id = get_next_id('candidates')
            now = datetime.now().isoformat()
            
            # MODIFIED: Add the new ID to the data dictionary
            formatted_data = {
                "id": new_id,
            ...
            with sqlite3.connect('recruitment.db') as conn:
                cursor = conn.cursor()
                # MODIFIED: Re-add 'id' to the INSERT statement
                cursor.execute(
                    '''
            ...
            formatted_data = {
                ...
                "certifications_required": json.dumps(data.get('certifications_required', [])),
...
def get_all_candidates() -> str:
    ...
    :returns: Formatted list of all candidates
    """
    try:
        conn = sqlite3.connect('recruitment.db')
        cursor = conn.cursor()
...
def get_all_bandos() -> str:
    """
    Retrieve all Bando di Gara from database
    
    :returns: Formatted list of all Bando di Gara
    """
    try:
        conn = sqlite3.connect('recruitment.db')
        cursor = conn.cursor()
...
def get_candidate_by_id(candidate_id: str) -> str:
    ...
    :param candidate_id: The candidate ID to search for
